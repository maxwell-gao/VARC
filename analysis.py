...
for name in tasks:
    ...
    for root in roots:
        ...
        if not os.path.exists(cur_task_save):
            print(
                f"Warning: Prediction file for task {name} not found at {cur_task_save}. Skipping."
            )
            continue
...
def analyze_data(answer_set, task_names, task_type, html_output: Path = HTML_OUTPUT):
    ...
    for task_name in task_names:
        ...
        for cur_index in answer_set[task_name]:
            ...
            for entry in majority_vote[:2]:
                matches_answer = (
                    has_ground_truth and entry["prediction"] == ground_truth
                )
            ...
            pass_at_1 = (
                len(majority_vote) > 0
                and majority_vote[0]["prediction"] == ground_truth
            )
    ...
    for rank, count in oracle_rank_sorted.items():
        print(
            f"Oracle rank {rank}: {count}. Cumulative: {(sum_correct + count) / (all_task_num):.4f}"
        )
        sum_correct += count
